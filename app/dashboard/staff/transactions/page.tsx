...
import { transactionsApi, type StkPushResponse, type ApiResponse } from '@/lib/api'
import {
  CreditCard, Search, CheckCircle, Clock, XCircle, RefreshCw,
  ArrowUpRight, WifiOff, Smartphone, Gauge, Calendar, Banknote
} from 'lucide-react'
...
export default function StaffTransactionsPage() {
  ...
  const fetchTransactions = useCallback(async () => {
    ...
    try {
      ...
      if (resp.success && resp.data) {
        ...
        setError(resp.message || 'Failed to load transactions')
      }
    } catch (err: any) {
  ...
  const filtered = transactions.filter(t => {
    const matchSearch = !searchQuery ||
      ...
      t.checkoutRequestId?.toLowerCase().includes(searchQuery.toLowerCase())
  ...
  return (
    <div className="min-h-screen">
      <Header title="Transactions" subtitle="Real-time payment records" user={user} />
      <div className="px-3 py-4 sm:px-4 sm:py-5 lg:px-6 lg:py-6 space-y-4 sm:space-y-5">
        {/* Stats */}
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
          {[
            { label: 'Total', value: stats.total, icon: CreditCard, color: 'water' },
            ...
            { label: 'Revenue', value: `KSh ${stats.totalAmount.toLocaleString()}`, icon: Banknote, color: 'water' },
          ].map((s, i) => (
          ...
          <div className="flex gap-2 flex-wrap">
            {(['all', 'SUCCESS', 'PENDING', 'FAILED'] as const).map(s => (
              <button key={s} onClick={() => setStatusFilter(s)}
                className={`px-3 py-2 rounded-lg text-xs font-medium capitalize ${statusFilter === s ? 'bg-water-500/20 text-water-400 border border-water-500/30' : 'bg-white/5 text-white/70 border border-white/10'}`}>
                {s === 'all' ? 'All' : s}
              </button>
            ))}
            <button onClick={fetchTransactions} className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white/70 hover:bg-white/10">
          ...
          <div className="glass-card p-6 border border-rose-500/30 bg-rose-500/10">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <button onClick={fetchTransactions} className="mt-2 text-sm text-rose-400 underline">Retry</button>
              </div>
            </div>
          </div>
        )}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/10">
                    {['Receipt', 'Meter', 'Phone', 'Amount', 'Status', 'Date'].map(h => (
                  ...
                  {filtered.map((t, i) => {
                    ...
                    return (
                      <tr key={t.id || i} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                        <td className="px-4 py-3 text-sm font-mono text-water-400">{t.mpesaReceiptNumber || t.checkoutRequestId?.substring(0, 16) || '—'}</td>
                        ...
                        <td className="px-4 py-3 text-sm font-semibold text-white">KSh {t.amount?.toLocaleString()}</td>
                        <td className="px-4 py-3">
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-xs font-medium ${ss.bg} ${ss.text}`}>
                ...
                return (
                  <div key={t.id || i} className="p-4 space-y-2">
                    ...
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-white/60">Meter: <span className="text-white font-mono">{t.meterNumber}</span></span>
                      <span className="text-white font-semibold">KSh {t.amount?.toLocaleString()}</span>
                    </div>
        ...
        {!loading && filtered.length === 0 && transactions.length > 0 && (
          <div className="glass-card p-12 text-center">
            ...
            <p className="text-white/60">No transactions match your search</p>
          </div>
        )}
      </div>
    </div>
